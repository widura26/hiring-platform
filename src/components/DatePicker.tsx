...
export default function DatePicker({ onDateSelect, setDob, initialDate, minDate, maxDate, label = "Date of birth",
        ...
        <div className="relative">
            <button type='button' onClick={() => setIsOpen(!isOpen)} className={`flex items-center justify-between w-full ${styleInput}`}>
                <div className="flex items-center gap-3">
                    ...
                    {/* <span className={`${selectedDate ? 'text-gray-900' : 'text-gray-500'}`}>{selectedDate ? formatDisplayDate(selectedDate) : placeholder}</span> */}
                    <input onChange={(e) => {if (setDob && selectedDate) setDob(selectedDate.toISOString)}} title='date-of-birth' name='date-of-birth' value={selectedDate ? formatDisplayDate(selectedDate) : placeholder} type="text" className={`focus:outline-none ${selectedDate ? 'text-gray-900' : 'text-gray-500'}`} />
                </div>
